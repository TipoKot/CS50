...
bool check_for_cycle(int winner, int loser)
{
    // Base case: If the loser is directly locked to the winner, it forms a cycle
    if (locked[loser][winner])
    {
        return true;
    }

    // Recursive case: Check if any candidate that the loser is locked to forms a cycle with the
    // winner
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[loser][i])
